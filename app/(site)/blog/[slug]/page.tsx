...
export async function generateMetadata({
  ...
  if (!post) {
    return {
      title: "Artikel tidak ditemukan | Fertechtive",
  ...
  const description =
    ...
    post.excerpt?.trim() ||
    `Baca artikel ${post.title} di Fertechtive.`;
...
(function(){
  ...
  function guess(txt){
    ...
    if(/\\b(fn\\s+[a-zA-Z_]+\\(|let\\s|println!\\(|::)\\b/.test(txt)){
      return "rust";
    }

    if(/\\b(function|const|let|=>|console\\.log)\\b/.test(txt)){
      return "javascript";
    }

    if(/\\b(def|import|self|print)\\b/.test(txt)){
      return "python";
    }

    if(/\\b(public|class|new)\\b/.test(txt)){
      return "java";
    }

    if(/\\b(#include|std::|printf)\\b/.test(txt)){
      return "cpp";
    }

    if(/\\bSELECT\\b|\\bFROM\\b|\\bWHERE\\b/i.test(txt)){
      return "sql";
    }
  ...
  function enhanceOne(code){
    ...
    /* Language badge */
    const lang =
      detectLangClass(code) ||
      guess(code.textContent || "") ||
      "text";
    ...
    bar.className = "pre-toolbar";

    const badge = document.createElement("span");
    badge.className = "pre-chip";
    badge.textContent = (lang || "TEXT").toUpperCase();

    const wrapBtn = document.createElement("button");
    wrapBtn.type = "button";
    wrapBtn.className = "pre-chip pre-btn";
    wrapBtn.textContent = "WRAP";

    wrapBtn.onclick = () => {
      const on = pre.getAttribute("data-wrap") === "on";

      pre.setAttribute("data-wrap", on ? "off" : "on");
      wrapBtn.textContent = on ? "NOWRAP" : "WRAP";
    };

    const copyBtn = document.createElement("button");
    copyBtn.type = "button";
    copyBtn.className = "pre-chip pre-btn";
    ...
    bar.append(badge, wrapBtn, copyBtn);
    pre.insertBefore(bar, code);

    /* Prism only when custom tokens are not present */
...
export default async function BlogDetailPage({
  ...
  const articleImage = post.coverUrl
    ? post.coverUrl.startsWith("http")
      ? post.coverUrl
      : `${SITE_URL}${post.coverUrl}`
    : `${SITE_URL}/default-blog.jpg`;
        ...
        ? [
            {
              ...
              item: `${SITE_URL}/blog?cat=${encodeURIComponent(
                firstCategory.slug
              )}`,
            },
          ]
      ...
      >
        ...
        >
          ...
          {firstCategory ? (
            <>
              <Link
                href={`/blog?cat=${encodeURIComponent(
                  firstCategory.slug
                )}`}
                className="hover:text-cyan-400 transition"
              >
